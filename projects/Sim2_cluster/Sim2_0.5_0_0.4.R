...
MLE_0.5_0_0.4_n1 = matrix(NA, nrow = N_sim, ncol = 3)
MLE_0.5_0_0.4_n2 = matrix(NA, nrow = N_sim, ncol = 3)
MLE_0.5_0_0.4_n3 = matrix(NA, nrow = N_sim, ncol = 3)
colnames(MLE_0.5_0_0.4_n1) = c("tau2", "sigma2", "phi")
colnames(MLE_0.5_0_0.4_n2) = c("tau2", "sigma2", "phi")
colnames(MLE_0.5_0_0.4_n3) = c("tau2", "sigma2", "phi")

t <- proc.time()
for (i in 1:N_sim){
  X_data = Sim_data_0.5["0", "0.5_0.4", i, 1:n1]
  optL <- optimx(phi0.5_2, neg_loglik_0.5_profile_0nug, 
                 X = X_data, D = D[1:n1, 1:n1], 
                 n = n1, method = c('L-BFGS-B'), lower = 0)
  sigma2_MLE <- sigma2_MLE_cal_0nug(optL$p1,  X = X_data, 
                               D = D[1:n1, 1:n1], n = n1)
  MLE_0.5_0_0.4_n1[i, ]<- c(0, sigma2_MLE, optL$p1)
}
proc.time() - t
...
t <- proc.time()
for (i in 1:N_sim){
  X_data = Sim_data_0.5["0", "0.5_0.4", i, 1:n2]
  optL <- optimx(phi0.5_2, neg_loglik_0.5_profile_0nug, 
                 X = X_data, D = D[1:n2, 1:n2], 
                 n = n2, method = c('L-BFGS-B'), lower = 0)
  sigma2_MLE <- sigma2_MLE_cal_0nug(optL$p1,  X = X_data, 
                                    D = D[1:n2, 1:n2], n = n2)
  MLE_0.5_0_0.4_n2[i, ]<- c(0, sigma2_MLE, optL$p1)
}
proc.time() - t
...
t <- proc.time()
for (i in 1:N_sim){
  X_data = Sim_data_0.5["0", "0.5_0.4", i, 1:n3]
  optL <- optimx(phi0.5_2, neg_loglik_0.5_profile_0nug, 
                 X = X_data, D = D[1:n3, 1:n3], 
                 n = n3, method = c('L-BFGS-B'), lower = 0)
  sigma2_MLE <- sigma2_MLE_cal_0nug(optL$p1,  X = X_data, 
                                    D = D[1:n3, 1:n3], n = n3)
  MLE_0.5_0_0.4_n3[i, ]<- c(0, sigma2_MLE, optL$p1)
}
proc.time() - t
# 10 for 233.465 seconds. Estimated time: 23346.5


save(list = c("MLE_0.5_0_0.4_n1", "MLE_0.5_0_0.4_n2", "MLE_0.5_0_0.4_n3"), 
     file = "./data/sim2_0.5_0_0.4results.RData", 
     envir = .GlobalEnv)
