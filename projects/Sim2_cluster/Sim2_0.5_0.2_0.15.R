...
MLE_0.5_0.2_0.15_n1 = matrix(NA, nrow = N_sim, ncol = 3)
MLE_0.5_0.2_0.15_n2 = matrix(NA, nrow = N_sim, ncol = 3)
MLE_0.5_0.2_0.15_n3 = matrix(NA, nrow = N_sim, ncol = 3)
colnames(MLE_0.5_0.2_0.15_n1) = c("tau2", "sigma2", "phi")
colnames(MLE_0.5_0.2_0.15_n2) = c("tau2", "sigma2", "phi")
colnames(MLE_0.5_0.2_0.15_n3) = c("tau2", "sigma2", "phi")

t <- proc.time()
for (i in 1:N_sim){
  X_data = Sim_data_0.5["0.2", "0.5_0.15", i, 1:n1]
  optL <- optimx(c(phi0.5_1, tau22), neg_loglik_0.5_profile,
                 X = X_data, D = D[1:n1, 1:n1], 
                 n = n1, method = c('L-BFGS-B'), lower = 0)
  sigma2_MLE <- sigma2_MLE_cal(optL$p1, optL$p2,  X = X_data, 
                               D = D[1:n1, 1:n1], n = n1)
  MLE_0.5_0.2_0.15_n1[i, ]<- c(sigma2_MLE * optL$p2, sigma2_MLE, optL$p1)
}
proc.time() - t
...
t <- proc.time()
for (i in 1:N_sim){
  X_data = Sim_data_0.5["0.2", "0.5_0.15", i, 1:n2]
  optL <- optimx(c(phi0.5_1, tau22), neg_loglik_0.5_profile,
                 X = X_data, D = D[1:n2, 1:n2], 
                 n = n2, method = c('L-BFGS-B'), lower = 0)
  sigma2_MLE <- sigma2_MLE_cal(optL$p1, optL$p2,  X = X_data, 
                               D = D[1:n2, 1:n2], n = n2)
  MLE_0.5_0.2_0.15_n2[i, ]<- c(sigma2_MLE * optL$p2, sigma2_MLE, optL$p1)
}
proc.time() - t
...
t <- proc.time()
for (i in 1:N_sim){
  X_data = Sim_data_0.5["0.2", "0.5_0.15", i, 1:n3]
  optL <- optimx(c(phi0.5_1, tau22), neg_loglik_0.5_profile,
                 X = X_data, D = D[1:n3, 1:n3], 
                 n = n3, method = c('L-BFGS-B'), lower = 0)
  sigma2_MLE <- sigma2_MLE_cal(optL$p1, optL$p2,  X = X_data, 
                               D = D[1:n3, 1:n3], n = n3)
  MLE_0.5_0.2_0.15_n3[i, ]<- c(sigma2_MLE * optL$p2, sigma2_MLE, optL$p1)
}
proc.time() - t
# 5 for 431.153 seconds. Estimated time: 86230.6 seconds


save(list = c("MLE_0.5_0.2_0.15_n1", "MLE_0.5_0.2_0.15_n2", "MLE_0.5_0.2_0.15_n3"),
     file = "./data/sim2_0.5_0.2_0.15results.RData", 
     envir = .GlobalEnv)
