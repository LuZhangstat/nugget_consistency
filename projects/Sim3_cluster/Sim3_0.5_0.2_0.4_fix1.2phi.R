...
phi_fix = 1.2 * phi0.5_2

MLE_0.5_0.2_0.4_fix1.2phi = array(NA, c(N_sim, 2, 8))
...
for (j in 1:length(Sam_N_list)){
  ...
  for (i in 1:N_sam){
    ...
    sigma2_MLE <- sigma2_MLE_cal(phi_fix, optL$p1,  X = X_data, 
                                 D = D[1:Sam_N_list[j], 1:Sam_N_list[j]], 
                                 n = Sam_N_list[j])
    MLE_0.5_0.2_0.4_fix1.2phi[i, , j]<- c(sigma2_MLE * optL$p1, sigma2_MLE)
  }
  cat(proc.time() - t, "\n")
}
proc.time() - t
...
save(list = c("MLE_0.5_0.2_0.4_fix1.2phi"), 
     file = "./data/sim3_MLE_0.5_0.2_0.4_fix1.2phiresults.RData", 
     envir = .GlobalEnv)
